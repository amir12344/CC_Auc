...
export default function EarlyAccessForm() {
  ...
  const welcomeContainerClasses =
    "relative flex flex-col justify-center bg-transparent mt-0 md:mt-0 border-none ring-0 px-4 py-5 rounded-2xl md:items-center md:py-8 md:px-6 backdrop-blur-md animate-fade-in-up";
  const earlyAccessBadgeClasses =
    "md:block absolute -top-3 md:-top-4 left-1/2 -translate-x-1/2 px-3 md:px-4 py-1 rounded-full bg-gradient-to-r from-[#102d21] to-[#43cd66] text-white text-xs font-semibold shadow-md tracking-wider uppercase z-10";
  const inputBaseClasses =
    "block w-full pr-3 py-2 md:py-2.5 text-sm border rounded-lg shadow-xs focus:outline-none focus:ring-[#43CD66] focus:border-[#43CD66] text-gray-900";
  ...
  const termsCheckboxClasses =
    "focus:ring-[#43CD66] h-4 w-4 text-[#43CD66] border-gray-300 rounded";
  const termsLabelSpecificClasses = "font-medium text-gray-700";
  const termsLinkClasses = "text-[#43CD66] hover:text-[#102D21]";
  const errorMessageContainerClasses =
    "flex items-start gap-3 bg-gradient-to-r from-red-50 to-red-50/70 border-l-4 border-red-500 text-red-800 px-4 py-3 rounded-md shadow-sm mt-3 mb-2 relative overflow-hidden";
  const errorIconContainerClasses = "relative";
  const errorAlertIconClasses =
    "h-4 w-4 md:h-5 md:w-5 text-red-500 mt-0.5 flex-shrink-0 transform transition-all duration-300 ease-in-out";
  const errorTextWrapperClasses = "flex-1 relative z-10";
  const errorDismissButtonClasses =
    "ml-2 text-red-400 hover:text-red-600 focus:outline-none relative z-10 transition-all duration-300 ease-in-out transform hover:scale-110";
  const errorDismissIconClasses =
    "h-3 w-3 md:h-4 md:w-4 transition-transform duration-300 hover:rotate-90";
  const submitButtonBaseClasses =
    "w-full bg-[#43CD66] hover:bg-[#3ab859] border border-[#1c1e21] hover:border-[#102D21] text-[#1C1E21] font-medium py-3.5 px-6 rounded-full transition-all duration-200 focus:outline-none text-sm text-base flex justify-center items-center";
  const loaderIconClasses =
    "animate-spin -ml-1 mr-2 h-3 w-3 md:h-4 md:w-4 text-white";
  const submitButtonTextClasses = "text-[#1C1E21] text-md md:text-lg";
  const headingClasses =
    "text-3xl text-center md:text-4xl mb-2 md:mb-4 font-bold";
  ...
  const subheadingMobileUnderlineClasses =
    "block md:hidden mx-auto h-1 w-27 md:w-22 bg-[#43CD66] rounded-full mt-1";
  const subheadingDesktopUnderlineClasses =
    "hidden md:block h-1 md:w-full bg-[#43CD66] rounded-full mt-1";
  const inputRowClasses = "flex flex-col md:flex-row gap-3 md:gap-6";
  const inputFieldWrapperClasses = "w-full md:w-1/2";
  const inputIconWrapperClasses =
    "absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none";
  const phonePrefixClasses =
    "absolute inset-Y-0 left-9 md:left-10 top-2.5 flex items-center text-[#43CD66] font-medium select-none pointer-events-none text-sm md:text-base";
  const termsLabelWrapperClasses = "ml-3 mt-0.5 md:mt-0 text-xs md:text-sm";
  const errorMessageShimmerBgClasses =
    "absolute inset-0 bg-red-100/40 rounded-r-md";
  ...
  const processForm = async (data: FormData) => {
    ...
    if (result.success) {
      ...
      toast({
        title: "Request Sent!",
        description:
          "We've received your early access application. We'll be in touch soon.",
      });
    ...
    <div className="mx-auto">
      ...
      <div className="mb-0 md:mb-4">
        <div className={welcomeContainerClasses}>
          <span className={earlyAccessBadgeClasses}>Early Access</span>
          <h2 className={headingClasses}>Secure Access to Exclusive Deals</h2>
          <div className={subheadingContainerClasses}>
            <span
              className={subheadingMobileClasses}
      ...
      <form onSubmit={handleSubmit(processForm)} className="mobile-card">
        ...
        <div className={inputRowClasses}>
          ...
          <div className={inputFieldWrapperClasses}>
            <label htmlFor="lastName" className={`${labelBaseClasses} text-md`}>
              Last Name
            </label>
            <div className="relative">
              <div className={inputIconWrapperClasses}>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <span className={phonePrefixClasses}>+1</span>
            <input
              type="tel"
              id="phoneNumber"
              inputMode="tel"
        ...
        <div className="flex items-start" style={{ marginBottom: "10px" }}>
          ...
          <div className={termsLabelWrapperClasses}>
            ...
            {errors.termsAccepted && (
              <p
                className={`${errorTextBaseClasses} text-xs font-medium md:text-sm`}
              >
        ...
        {errorMessage && (
          ...
          >
            ...
            >
              <span className={errorMessageTextClasses}>{errorMessage}</span>
            </div>
            <button
              type="button"
            ...
            >
              <X className={errorDismissIconClasses} aria-hidden="true" />
            </button>
          </div>
        )}
          ...
          <button
            ...
            onClick={() => {
              if (
                ...
                window.rdt("track", "EarlyAccessButtonClick", {
                  buttonName: "ReserveAccessEarlyAccessForm",
                });
              }
            }}
            className={`${submitButtonBaseClasses} ${!isValid || isSubmitting || submissionSucceeded ? "cursor-not-allowed opacity-70" : "cursor-pointer"}`}
          >
            {isSubmitting || submissionSucceeded ? (
              <>
                <Loader2 className={loaderIconClasses} aria-hidden="true" />
                Processing...
              </>
            ) : (
              <span className={submitButtonTextClasses}>
      ...
      <div className="mt-0 text-center text-xs text-gray-600 md:mt-6 md:block md:text-sm">
      ...
      <div className="mt-3 text-center text-xs text-gray-600 md:mt-6 md:text-sm">
