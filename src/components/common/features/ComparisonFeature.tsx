...
export const ComparisonFeature: React.FC<ComparisonFeatureProps> = ({
  ...
  const textColor =
    background === "dark" ? "text-[#F1E9DE]" : "text-text-secondary";
  const headingColor =
    background === "dark" ? "text-primary" : "text-text-primary";
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-20 text-center">
          {subtitle && (
            <motion.p
              className={`text-lg font-medium tracking-wider uppercase ${headingColor}`}
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-16">
          ...
          <div>
            <div className="mb-8 flex items-center">
              <div
                className={`h-10 w-2 ${problemLineColor} mr-3 rounded-r-md`}
              ></div>
              <div
                className={`flex items-center px-4 py-2 ${problemBgColor} rounded-lg shadow-xs`}
              >
                {beforeIcon || (
                  <div className={`mr-2 ${problemIconColor} h-5 w-5`} />
                )}
            ...
            <div className="relative">
              {/* Vertical line */}
              <div
                className={`absolute top-0 bottom-0 left-[19px] w-[1px] ${problemVerticalLineColor}`}
              ></div>
                ...
                {items.map((item, index) => (
                  ...
                  >
                    <div className="absolute top-1.5 left-0">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-full ${problemCircleBgColor} border`}
                      >
                    ...
                    <div className="pl-16">
                      <h4
                        className={`text-xl font-semibold ${headingColor} mb-2`}
                      >
                        {item.problem.title}
                      </h4>
          ...
          <div>
            <div className="mb-8 flex items-center">
              <div
                className={`h-10 w-2 ${solutionLineColor} mr-3 rounded-r-md`}
              ></div>
              <div
                className={`flex items-center px-4 py-2 ${solutionBgColor} rounded-lg shadow-xs`}
              >
                {afterIcon || (
                  <div className={`mr-2 ${solutionIconColor} h-5 w-5`} />
                )}
                <h3 className={`text-2xl font-semibold ${headingColor}`}>
                  {afterLabel}
                </h3>
            ...
            <div className="relative">
              {/* Vertical line */}
              <div
                className={`absolute top-0 bottom-0 left-[19px] w-[1px] ${solutionVerticalLineColor}`}
              ></div>
              ...
              <div className="space-y-14">
                {items.map((item, index) => (
                  ...
                  >
                    <div className="absolute top-1.5 left-0">
                      <div
                        className={`flex h-10 w-10 items-center justify-center rounded-full ${solutionCircleBgColor} border`}
                      >
