    ...
    if (filterKey.startsWith("specialEvent-")) {
      ...
      if (currentEvents) {
        const events = currentEvents
          .split(",")
          .filter((event) => event !== filterValue);
        if (events.length > 0) {
    ...
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      {activeFilters.map((filter) => (
        <Badge
          key={filter.key}
          variant="secondary"
          className="flex items-center gap-1 px-3 py-1 text-sm transition-colors hover:bg-gray-200"
        >
          ...
          >
            <X className="h-3 w-3 transition-colors hover:text-red-500" />
          </Button>
        </Badge>
      ))}
      ...
      {activeFilters.length > 1 && (
        <Button
          ...
          className="h-auto px-2 py-1 text-xs text-gray-600 hover:border-red-300 hover:text-red-600"
        >
          Clear All
        </Button>
      )}
