...
const CATEGORY_IMAGES: Record<string, string> = {
  Electronics:
    "https://images.unsplash.com/photo-1596460107916-430662021049?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=870&q=80", // TV
  Appliances:
    "https://images.unsplash.com/photo-1622434641406-a158123450f9?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=704&q=80", // Dyson vacuum
  "Fashion & Accessories":
    "https://images.unsplash.com/photo-1667284152823-0b07a791fb79?q=80&w=2126&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Watch
  Footwear:
    "https://images.unsplash.com/photo-1563635419376-78d400e5588e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Hiking boots
  "Beauty & Personal Care":
    "https://images.unsplash.com/photo-1618120508902-c8d05e7985ee?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D", // Skincare
  "Home & Garden":
    "https://images.unsplash.com/photo-1583847268964-b28dc8f51f92?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&q=80", // Home goods
  Furniture:
    "https://images.unsplash.com/photo-1555041469-a586c61ea9bc?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&q=80", // Furniture
  "Office & Business":
    "https://images.unsplash.com/photo-1497493292307-31c376b6e479?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=500&q=80", // Office supplies
  "General Merchandise":
    "https://images.unsplash.com/photo-1581783898377-1c85bf937427?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=580&q=80", // General
};
...
const generateCollections = (): Collection[] => {
  ...
  const categoryMap = new Map<
    string,
    { products: Product[]; originalCategories: Set<string> }
  >();
  ...
  const collections: Collection[] = Array.from(categoryMap.entries()).map(
    ([category, data]) => ({
  ...
  switch (slug) {
    ...
    default:
      ...
      if (consolidatedCategory) {
        ...
        products = allMockProducts.filter(
          (product) =>
...
export const searchProducts = async (
  ...
  let results = allMockProducts.filter(
    (product) =>
...
const applyFilters = (
  ...
  if (filters.discountRange) {
    ...
    filtered = filtered.filter(
      (p) => p.discount && p.discount >= min && p.discount <= max
    );
  }

  if (filters.inStock) {
    filtered = filtered.filter((p) => p.unitsAvailable && p.unitsAvailable > 0);
  }

  // Brand/retailer filtering
  if (filters.retailer) {
    const retailerFilters = filters.retailer
      .split(",")
      .map((r) => r.toLowerCase().trim());
    filtered = filtered.filter(
      (p) =>
        p.retailer &&
        retailerFilters.some((filter) =>
      ...
      return events.some((event) => {
        switch (event.toLowerCase()) {
          ...
          default:
            ...
            return (
              p.label?.toLowerCase().includes(event.toLowerCase()) ||
