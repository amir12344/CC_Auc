...
export const getProducts = async (options: {
  ...
  console.warn("🔍 Fetching products with options:", {
    filters,
    page,
    pageSize,
    sortBy,
  ...
  const endTime = performance.now();
  console.warn(
    `✅ Products fetched in ${(endTime - startTime).toFixed(2)}ms - ${paginatedProducts.length}/${totalProducts} products`
  );
...
export const searchProducts = async (
  ...
  if (
    !normalizedQuery ||
    normalizedQuery.length < SERVICE_CONFIG.SEARCH_MIN_LENGTH
  ) {
  ...
  console.warn(`🔍 Searching products for: "${query}"`);
  ...
  console.warn(
    `✅ Search completed in ${executionTime.toFixed(2)}ms - ${paginatedResults.length}/${totalResults} results`
  );
...
const applyFilters = (
  ...
  if (filters.condition) {
    filtered = filtered.filter((p) => {
      const productCondition =
        p.condition || (p.isRefurbished ? "refurbished" : "new");
  ...
  if (filters.specialEvents?.length || filters.specialEvent) {
    const events =
      ...
      (Array.isArray(filters.specialEvent)
...
const parseFiltersFromParams = (searchParams: {
  ...
  if (searchParams.price) {
    const priceStr = Array.isArray(searchParams.price)
  ...
  if (searchParams.discount) {
    const discountStr = Array.isArray(searchParams.discount)
      ? searchParams.discount[0]
  ...
  if (searchParams.specialEvent) {
    const eventsStr = Array.isArray(searchParams.specialEvent)
      ? searchParams.specialEvent[0]
      : searchParams.specialEvent;
