...
interface NavigationContextType {
  ...
  currentPath: string
  currentSection: 'homepage' | 'collections' | 'marketplace' | 'search' | 'product'
...
export const NavigationProvider = ({ children }: { children: ReactNode }) => {
  ...
  const navigateToCollection = (category: string, filters?: ProductFilters) => {
    ...
    if (filters) {
      ...
      if (filters.discountRange) {
        params.set('discount', `${filters.discountRange[0]}-${filters.discountRange[1]}`)
      }
  ...
  const updateFilters = (newFilters: Partial<ProductFilters>) => {
    ...
    if (mergedFilters.priceRange) {
      params.set('price', `${mergedFilters.priceRange[0]}-${mergedFilters.priceRange[1]}`)
    }
    if (mergedFilters.condition) {
      params.set('condition', mergedFilters.condition)
    }
    if (mergedFilters.discountRange) {
      params.set('discount', `${mergedFilters.discountRange[0]}-${mergedFilters.discountRange[1]}`)
    }
    ...
    switch (section) {
      ...
      case 'product':
        ...
        if (source === 'collections' && category) {
          breadcrumbs.push({ label: 'Collections', href: '/collections' })
          breadcrumbs.push({ label: formatCategoryName(category), href: `/collections/${category}` })
        } else if (source === 'search' && query) {
          breadcrumbs.push({ label: `Search: "${query}"`, href: `/search?q=${query}` })
        } else {
          breadcrumbs.push({ label: 'Marketplace', href: '/marketplace' })
        }
        breadcrumbs.push({
